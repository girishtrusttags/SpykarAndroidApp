...
class WashCodeScanActivity : BaseActivity(),BarcodeReader.BarcodeReaderListener {
    val TAG = WashCodeScanActivity::class.java.simpleName
...
//            if(strCode.equals("mrp")) {
//
//                val intent = Intent(this@WashCodeScanActivity, WashCodeScanActivity::class.java)
...
//            }
        ...
        imgClear.setOnClickListener {
            PreferenceConnector.writeInteger(this@WashCodeScanActivity, PreferenceConnector.MAPPED_CODE, 0)
            Toast.makeText(
                    this@WashCodeScanActivity, "Session cleared",
                    Toast.LENGTH_SHORT
            ).show()
            val intent = Intent(this@WashCodeScanActivity, HomeActivity::class.java)
...
//        barcodeReader.playBeep()
        ...
        runOnUiThread {
            ...
            if(strQrCode.equals(mrpTagCode,true)){
                return@runOnUiThread
            }
            barcodeReader.pauseScanning()
...
//            }
    ...
    private fun mapCodes(mrpTagCode: String, washCareCode: String){
        ...
        if(FLAVOR == "pantaloons") {
            base64DataJsonStringToken.put("qr_code", mrpTagCode)
            base64DataJsonStringToken.put("ean_code", washCareCode)

        }else{
            base64DataJsonStringToken.put("mrpTagCode", mrpTagCode)
            base64DataJsonStringToken.put("washCareCode", washCareCode)
        }
        ...
        if (FLAVOR == "pantaloons"){
            if(strType.equals("map_code"))
                reqObserver = apiService.mapPantaloonsCodes(requestBody)
            else
                reqObserver = apiService.reMapPantaloonsCodes(requestBody)
        }else{
            if(strType.equals("map_code"))
                reqObserver = apiService.mapCodes(requestBody)
            else
...
//                            onBackPressed()
//                            finish()
                            val intent = Intent(this@WashCodeScanActivity, MRPScanCodeActivity::class.java)
...
//                    listenerResponse.onError(e.message.toString())
    ...
    private fun sessionOut(){
        Toast.makeText(
                this@WashCodeScanActivity,"Invalid Token or Key",
                Toast.LENGTH_SHORT
        ).show()
        val intent = Intent(this@WashCodeScanActivity, LoginActivity::class.java)
        intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP)
        startActivity(intent)
        finish()
    }
